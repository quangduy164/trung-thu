...
{frame:2,who:'an',text:'Em sẽ để dành chiếc bánh đẹp nhất cho anh Hai. Năm nay anh nhất định sẽ về.'},
...
function next(){if(!ready||filmEl.dataset.beat==='choices')return;if(typing){clearTimeout(timer);const target=story[index].who==='an'?speech:caption;target.textContent=current;pos=current.length;typing=false;return}if(index<story.length-1){index++;render()}else if(chapter===1){document.querySelector('#end h2').textContent='Chương 1 kết thúc';document.querySelector('#end p').textContent='An bước theo đom đóm vào rừng tre.';document.querySelector('#end button').textContent='Bước vào Chương 2';document.querySelector('#end button').onclick=startChapter2;document.querySelector('#end').classList.add('show')}else{document.querySelector('#end h2').textContent='Chương 2 kết thúc';document.querySelector('#end p').textContent='Cánh cửa dưới mặt hồ đã mở.';document.querySelector('#end button').textContent='Xem lại từ đầu';document.querySelector('#end button').onclick=()=>location.reload();document.querySelector('#end').classList.add('show')}}
function begin(e){e.stopPropagation();if(chapter2Intro){chapter2Intro=false;started=true;intro.classList.add('hide');startChapter2();return}if(started)return;started=true;intro.classList.add('hide');countdown.classList.add('show');filmEl.classList.add('counting-down');let left=5;document.querySelector('#count').textContent=left;const tick=setInterval(()=>{left--;if(left>0)document.querySelector('#count').textContent=left;else{clearInterval(tick);countdown.classList.remove('show');filmEl.classList.remove('counting-down');ready=true;render()}},1000)}
document.querySelector('#start').addEventListener('click',begin);filmEl.addEventListener('click',e=>{if(e.target.closest('button,.intro,.end'))return;if((filmEl.dataset.beat==='moon-clues'||filmEl.dataset.beat==='moon-scattered')&&moonFound<4)return;next()});document.addEventListener('keydown',e=>{if(e.code==='Space'){e.preventDefault();if(!started)begin(e);else if(!((filmEl.dataset.beat==='moon-clues'||filmEl.dataset.beat==='moon-scattered')&&moonFound<4))next()}});
